...
export const login = async (req: Request, res: Response) => {
    ...
    if (!userData) {
        return res.status(404).send({ message: 'The user you are trying to login does not exist' })
    } else {
        user = {
            ...user,
            _id: userData._id,
...
const generateAccessToken = (user: IUser) => {
    ...
    return jwt.sign({ name: user.name, email: user.email }, JWT_SECRET, { expiresIn: 86400, subject: user._id?.toString(), issuer: `http://${process.env.API_URL ?? "localhost"}:${process.env.PORT ?? 4000}` });
}


const checkIfPasswordMatches = async (password: string, hashedPassword: string): Promise<boolean> => {
